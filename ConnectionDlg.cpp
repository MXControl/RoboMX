...
void CConnectionDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_USERNAME, m_eName);
	DDX_Check(pDX, IDC_USE_WPNNAME, m_bWPN);
...
BOOL CConnectionDlg::OnInitDialog() 
{
	...
	//m_eName.EnableWindow(!m_bWPN);
	m_strName = g_sSettings.GetNickname();

	Util::MakeValidUserName(m_strName, m_bWPN, GetApp()->GetMainFrame()->m_wClientID);
	...
	//m_mToolTip.AddControlInfo(IDC_ROOM, IDC_TOOL_353LOOPBACK);
	m_mToolTip.AddTool(GetDlgItem(IDC_ROOM), IDC_TOOL_ROOM, 0, 0, 0);
	m_mToolTip.AddTool(GetDlgItem(IDC_USE_WPNNAME), IDC_WPNNAME, 0, 0, 0);
...
void CConnectionDlg::OnOK() 
{
	...
	int n = m_cbRoom.FindString(-1, Util::GetBaseRoomName(m_strRoom));

	if(n >= 0){

		m_cbRoom.DeleteString(n);
	}
	...
	for(int i = 0; (i < m_cbRoom.GetCount()) && (i < 20) ; i++){
...
void CConnectionDlg::FixRoomName(CString& strRoom)
{
	...
	if(n > 0){
		...
		strSuffix = strRoom.Mid(n);
    }
	else return;

	strSuffix.Remove('_');
	
	if(strSuffix.GetLength() == 12) return; // this is the normal case

	// if we are here the rommane has format
	// room_XXX.XXX.XXX.XXX:XXXX

	CTokenizer token(strSuffix, ".:");
	...
	strSuffix.Format("%02X%02X%02X%02X%04X", nD, nC, nB, nA, nPort);

	strRoom = strTmp + strSuffix;
}

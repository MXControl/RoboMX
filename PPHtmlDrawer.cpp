				...
				if (!m_defStyle.strTag.IsEmpty())
				{
					...
					else 
					{
						...
						case TAG_BITMAP:
							...
							if (si.nIdRes || si.nIdDll || si.nHandle || (!si.strSrcFile.IsEmpty() && (si.strSrcFile.Find(".bmp", 0) > 0)))
							{
							...
							else if(!si.strSrcFile.IsEmpty() && (si.strSrcFile.Find(".bmp", 0) < 0))
							{

								CImage image;
								if(image.Load(si.strSrcFile) == S_OK){
									...
									if (si.nWidth && si.nHeight && (MODE_DRAW == m_nNumPass))
									{
										...
										//ENG: Drawing an image
										//RUS: Рисование изображения
										m_drawmanager.DrawBitmap(m_hDC, ptOutput.x, y, si.nWidth, si.nHeight, (HBITMAP)image, 
											si.bUseMask, si.crMask, si.nStyles, 
											bShadow, 
											m_szOffsetShadow.cx, m_szOffsetShadow.cy, 
											m_szDepthShadow.cx, m_szDepthShadow.cy, 
...
void CPPHtmlDrawer::SetDefaultCursor()
{
	if (m_hLinkCursor == NULL)                // No cursor handle - load our own
    {
		...
		m_hLinkCursor = GetApp()->LoadCursor(IDC_HAND);

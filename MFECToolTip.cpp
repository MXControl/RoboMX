...
void CMFECToolTip::CalculateHeightAndWidth(CStringArray& straInfos)
{
	...
	for ( i=0; i < sizeof( m_aTabStobs ) / sizeof( m_aTabStobs[0] ); i++ )
	{
...
{
	// TODO, make offset programmable by the client
	const int nVerticalOffset = 10;
...
BOOL CMFECToolTip::AddControlInfo(UINT contolID, CString strLang, COLORREF back, COLORREF text)
{
	...
	for ( nStrPos = 0; nStrPos < strLang.GetLength(); nStrPos++ )
	{
		if ( strLang[ nStrPos ] == '\n' )
		{
			strArray.Add( strLang.Mid( nStart, nStrPos - nStart ) );
			nStart = nStrPos + 1;
		}
	}

	strArray.Add( strLang.Mid( nStart, strLang.GetLength() - nStart ) );

...
CAboutCtrl::CAboutCtrl()
{
	...
	m_strCredits.Format("\t%s\n[Protocoll-version 3.53]\n \n"
				   "\rEngineering Lead /\n\rCore Development:\n"
				   "Thees 'Bender' Schwab,\n \n"
				   "\rAdditional Programming:\n"
				   "The Shareaza Team (www.shareaza.com),\n"
				   "[Nushi]@[2SN],\n"
				   "Eugene Pustovoyt,\n"
				   "Pablo van der Meer,\n"
				   "N. T. Almond,\n"
				   ...
				   "\r'Nörgeln':\n"
				   "Jann Peterecki \n \n"
				   "\rCopyright © 2002-2004 Contributors.\n"
				   "\rhttp:\\\\mxcontrol.soureforge.net\n \n"
				   "Distributed under the terms of the\n"
				   "GNU GENERAL PUBLIC LINCESE\n"
				   "Version 2.0 or later.\n \n"
				   "MXControl - Made with 10%% more love\n"
				   "then the next leading brand\n \n"
				   "\t     [:] + {:} \n \n \n"
						   , STR_VERSION_DLG);
...
void CAboutCtrl::Initialize()
{
	...
	LoadPicture(IDB_ROBOMX, m_hLogoDC, m_cxLogo, m_cyLogo, pDC->m_hDC);
	// Load Backgroundpicture and creates background DC
	LoadPicture(IDB_ABOUTBG, m_hBackgroundDC, cx, cy, pDC->m_hDC); 
	// create work area
...
void CAboutCtrl::AnimateLogo()
{
	...
	for (int i = 1; i < m_cxLogo; i++)
	{
		...
		BitBlt(m_hMemDC, (int)(cos(deg2rad(m_xAngle + i)) * (m_cxLogo / 4.25) + m_xPos), 
								 ...
								 1, m_cyLogo, m_hLogoDC, i, 0, MERGEPAINT);
	}
	...
	BitBlt(m_hMemDC, 194, m_nCounter--, m_cxData, yPos, m_hCreditsDC, 0, 0,MERGEPAINT);
...
void CAboutCtrl::LoadCredits(HDC &hDestinationDC, int nWidth, int nHeight, HDC hDC)
{
	...
	else
	{
		...
		while(AfxExtractSubString(strSub, m_strCredits, nCount++, '\n'))
		{
			...
			{
				case '\t':	// title
					oldColor = SetTextColor(hDestinationDC, RGB(200,100,0));
					...
					break;
				case '\r':	// bold
					oldColor = SetTextColor(hDestinationDC, RGB(10,10,200));

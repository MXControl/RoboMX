...
{ 
	PPTRACE(_T("CPPToolTip::SendNotify()\n")); 
...
{
	PPTRACE(_T("CPPToolTip::IsNotify\n"));
...
BOOL CPPToolTip::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	...
	PPTRACE (_T("CPPToolTip::OnSetCursor(x=%d, y=%d)\n"), ptClient.x, ptClient.y);
...
{
	PPTRACE (_T("CPPToolTip::OnRepaintWindow()\n"));
...
{
	PPTRACE (_T("CPPToolTip::OnRedrawTooltip(Transparency = %d)\n"), nTransparency);
...
BOOL CPPToolTip::RelayEvent(MSG* pMsg)
{
	...
	case WM_SETFOCUS:
		...
		break;
	case WM_LBUTTONDOWN:
		PPTRACE(_T("CPPToolTip::WM_LBUTTONDOWN\n"));
...
{
	PPTRACE (_T("CPPToolTip::SetNewTooltip(hWnd=0x%08X, CRect(left=%d, top=%d, right=%d, bottom=%d), nID=%d)\n"), 
...
void CPPToolTip::OnTimer(UINT nIDEvent) 
{
	...
	case TIMER_SHOW:
		...
		if ((pt.x != m_ptOriginal.x) || (pt.y != m_ptOriginal.y))
		{
			...
			PPTRACE(_T("OnTimerShow(HideTooltip)\n"));
			HideTooltip();
		}
		else if (PPTOOLTIP_STATE_HIDEN == m_nTooltipState)
		{
			PPTRACE(_T("OnTimerShow(Showing)\n"));
		...
		break;
	case TIMER_SHOWING:
		PPTRACE(_T("OnTimerFadeIn(Current Transparency=%d )\n"), m_dwCurTransparency);
	...
	case TIMER_HIDE:
		...
		break;
	case TIMER_HIDING:
		PPTRACE(_T("OnTimerFadeOut(Current Transparency=%d)\n"), m_dwCurTransparency);
...
{
	PPTRACE (_T("CPPToolTip::HideTooltip(CurState=%d)\n"), m_nTooltipState);
...
{
	PPTRACE (_T("CPPToolTip::SetAutoPopTimer()\n"));
...
{
	PPTRACE (_T("CPPToolTip::KillTimers()\n"));
	if (dwIdTimer == NULL)
	{
		KillTimer(TIMER_SHOW);
		KillTimer(TIMER_HIDE);
...
{
	PPTRACE (_T("CPPToolTip::PrepareDisplayTooltip()\n"));
...
{
	PPTRACE(_T("OutputTooltipOnScreen()\n"));
	CRect rect = m_rcBoundsTooltip;
...
{
	PPTRACE(_T("CPPToolTip::AddTool(hWnd=0x%08X)\n"), pWnd->GetSafeHwnd());
	ASSERT (pWnd);
...
{
	PPTRACE (_T("CPPToolTip::RemoveTool(hWnd=0x%08X)\n"), pWnd->GetSafeHwnd());
	ASSERT(pWnd);
...
{
	PPTRACE (_T("CPPToolTip::RemoveAllTools()\n"));
...
{
	PPTRACE (_T("CPPToolTip::AddToolBar(hWnd=0x%08X)\n"), pBar->GetSafeHwnd());
	ASSERT(pBar);
...
{
	PPTRACE(_T("CPPToolTip::SetCallbackHyperlink()\n"));
...
{
	PPTRACE(_T("CPPToolTip::SetNotify\n"));
...
{
	PPTRACE(_T("CPPToolTip::SetSize(nSizeIndex = %d, nValue = %d)\n"), nSizeIndex, nValue);
...
{
	PPTRACE(_T("CPPToolTip::GetSize(nSizeIndex = %d)\n"), nSizeIndex);
	if (nSizeIndex >= PPTTSZ_MAX_SIZES)
		return 0;
...
{
	PPTRACE(_T("CPPToolTip::SetDefaultSizes()\n"));
...
{
	PPTRACE(_T("CPPToolTip::SetDirection(nDirection = %d)\n"), dwDirection);
...
{
	PPTRACE(_T("CPPToolTip::GetDirection()\n"));
...
{
	PPTRACE(_T("CPPToolTip::ShowHelpTooltip\n"));

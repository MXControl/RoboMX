...
// Operations
public:
	BOOL				AddButton( CWnd* wnd, WORD wIcon = 0, LPCTSTR lpszImage = NULL );
...
// Implementation
public:
	void BroadcastMessage(UINT uMsg, WPARAM wParam, LPARAM lParam);
...
protected:
	...
	afx_msg LRESULT OnRClickBtn(WPARAM wParam, LPARAM lParam);
	afx_msg void OnDocmenuActivate();
	afx_msg void OnDocmenuClose();
	afx_msg void OnDocmenuMoveleft();
	afx_msg void OnDocmenuMoveright();
	DECLARE_MESSAGE_MAP()
...
public:
	...
	CView* GetPMView(CString strTitle);
	afx_msg void OnUpdateDocmenuMoveleft(CCmdUI *pCmdUI);
	afx_msg void OnUpdateDocmenuMoveright(CCmdUI *pCmdUI);
	void SwitchToNext(void);
	void SwitchToPrevious(void);
	afx_msg void OnDocmenuSetchannelpicture();
	afx_msg void OnUpdateDocmenuSetchannelpicture(CCmdUI *pCmdUI);
	void DisplayBtnTool(HWND hWnd, CString strText);
	void CloseChannellist(DWORD dwID);
};

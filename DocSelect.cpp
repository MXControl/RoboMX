...
#define DS_WIDTH				110		// Default is 110.
...
BEGIN_MESSAGE_MAP(CDocSelector, CControlBar)
	...
	ON_MESSAGE(SWM_RCLICK, OnRClickBtn)
	ON_MESSAGE(ID_DS_POPUP_BUTTON, OnDSPopup)
	ON_COMMAND(ID_DOCMENU_ACTIVATE, OnDocmenuActivate)
	ON_COMMAND(ID_DOCMENU_CLOSE, OnDocmenuClose)
	ON_COMMAND(ID_DOCMENU_MOVELEFT, OnDocmenuMoveleft)
	ON_COMMAND(ID_DOCMENU_MOVERIGHT, OnDocmenuMoveright)
	ON_UPDATE_COMMAND_UI(ID_DOCMENU_MOVELEFT, OnUpdateDocmenuMoveleft)
	ON_UPDATE_COMMAND_UI(ID_DOCMENU_MOVERIGHT, OnUpdateDocmenuMoveright)
	ON_COMMAND(ID_DOCMENU_SETCHANNELPICTURE, OnDocmenuSetchannelpicture)
	ON_UPDATE_COMMAND_UI(ID_DOCMENU_SETCHANNELPICTURE, OnUpdateDocmenuSetchannelpicture)
END_MESSAGE_MAP()
...
void CDocSelector::InputAll(CString strText)
{
	...
	BroadcastMessage(UWM_TEXT_INPUT, 1, (LPARAM)(LPCTSTR)strText);
}
...
void CDocSelector::OnDocmenuMoveleft()
{

	if((m_iRID >= 0) && (m_iRID < m_Buttons.GetSize()) && (m_Buttons.GetSize() > 1))
	{

		int iNew = m_iRID-1 >= 0 ? m_iRID-1 : m_Buttons.GetSize()-1;
...
LRESULT CDocSelector::OnRClickBtn(WPARAM wParam, LPARAM lParam)
{


	m_iRID = ((CSwitcherButton*)wParam)->m_iID;
	POINT point;
	GetCursorPos(&point);

	CMenu mContextMenu;
	mContextMenu.LoadMenu(IDR_DOCMENU);

	//if(strstr(((CSwitcherButton*)wParam)->m_AttachedView->GetRuntimeClass()->m_lpszClassName, "CChatView") == 0){

		//mContextMenu.GetSubMenu(0)->RemoveMenu(MF_BYCOMMAND, ID_DOCMENU_SETCHANNELPICTURE);
		//mContextMenu.RemoveMenu(MF_BYPOSITION, 6);
	//}
...
void CDocSelector::OnUpdateDocmenuMoveleft(CCmdUI *pCmdUI)
{

	pCmdUI->Enable(m_Buttons.GetSize() > 1);
}

void CDocSelector::OnUpdateDocmenuMoveright(CCmdUI *pCmdUI)
{

	pCmdUI->Enable(m_Buttons.GetSize() > 1);
}

void CDocSelector::SwitchToNext(void)
{

	if(m_Buttons.GetCount() <= 1) return;

	((CSwitcherButton *)m_Buttons.GetAt(m_iSelectedButton))->Unselect();
	int nSel = 0;
	if(m_iSelectedButton + 1 < m_Buttons.GetSize()){

		nSel = m_iSelectedButton + 1;
	}

	::SendMessage(m_hWnd, SWM_SELCHANGE, (WPARAM)m_Buttons.GetAt(nSel), 0);
}

void CDocSelector::SwitchToPrevious(void)
{
	...
	int nSel = m_Buttons.GetSize() - 1;

	if(m_iSelectedButton - 1 >= 0){
...
void CDocSelector::DisplayBtnTool(HWND hWnd, CString strText)
{

	for(int i = 0; i < m_Buttons.GetSize(); i++){

		if(((CSwitcherButton *)m_Buttons[i])->m_AttachedView->m_hWnd == hWnd){

			((CSwitcherButton *)m_Buttons[i])->DisplayTool(strText);
			break;
		}
	}
}

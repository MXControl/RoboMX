...
#define MAX_HARDDISK 24
static char HardDiskLetters[MAX_HARDDISK][4]={
	"C:\\",	"D:\\",	"E:\\",	"F:\\",	"G:\\",	"H:\\",
	"I:\\",	"J:\\",	"K:\\",	"L:\\",	"M:\\",	"N:\\",
	"O:\\",	"P:\\",	"Q:\\",	"E:\\",	"S:\\",	"T:\\",
	"U:\\",	"V:\\",	"W:\\",	"X:\\",	"Y:\\",	"Z:\\" };
...
CString CSystemInfo::GetOnlineString(void)
{

	DWORD dwOnline = GetTickCount()-m_dwStartupTick;
	CString strOnline;

	DWORD dwHours = 0, dwMinutes = 0, dwSeconds = dwOnline / 1000;
	...
	strOnline.Format("%d:%02d.%02d", dwHours, dwMinutes, dwSeconds);

	return strOnline;
}

CString CSystemInfo::GetTotalRAM(void)
{
	...
	strRam.Format("%ldMB",(DWORD) ceil((long double)memoryStatus.dwTotalPhys/1024/1024));
	
	return strRam;
}

CString CSystemInfo::GetPCHostname(void)
{
	...
	if(GetComputerName(cComputerName, &dwSize) != 0){

		strHost = cComputerName;
	}
	else{
...
CString CSystemInfo::GetHardiskInfo(void)
{

	ULARGE_INTEGER nAvailableToCaller, nDisk, nFree;
	CString strResult, strTemp;

	for(int nCounter=0; nCounter < MAX_HARDDISK; nCounter++)
	{
		if(GetDriveType(HardDiskLetters[nCounter]) == DRIVE_FIXED)
		{
			if (GetDiskFreeSpaceEx(HardDiskLetters[nCounter], &nAvailableToCaller,
									&nDisk, &nFree))
			{
				strTemp.Format("[%s> ", HardDiskLetters[nCounter]);
				strResult += strTemp;

				strTemp.Format("Total size: %I64dGB, ", nDisk.QuadPart/1024/1024/1024);
				strResult += strTemp;

				ULONGLONG nUsed = nDisk.QuadPart - nFree.QuadPart;
				strTemp.Format("Used: %I64dGB ", nUsed/1024/1024/1024);
				strResult += strTemp;
				
				int nFreePerc = (int)(((float)nFree.QuadPart / (float)nDisk.QuadPart) * 100);
				strTemp.Format("Free: %I64dGB (%d%%) ", nFree.QuadPart/1024/1024/1024, nFreePerc);
				strResult += strTemp;
			}

			char cLabel[256]= "";
			char cFS[256]   = "";
			DWORD dwSerialNumber = 0, dwMaxFileLen = 0, dwFileSysFlag = 0;
			
			if (GetVolumeInformation(HardDiskLetters[nCounter], cLabel, 256, &dwSerialNumber,
				&dwMaxFileLen, &dwFileSysFlag, cFS, 256))
			{

				strTemp.Format(", %s]  ", cFS);
				strResult += strTemp;
			}
		}
	}

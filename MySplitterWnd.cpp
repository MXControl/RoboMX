...
CMySplitterWnd::CMySplitterWnd(int nPanes, UINT nOrientation, int nMinSize, int nBarThickness):
	m_nPanes(nPanes),
...
BOOL CMySplitterWnd::CreatePane(int nIndex, CWnd* pPaneWnd, DWORD dwStyle, DWORD dwExStyle, LPCTSTR lpszClassName)
{
	CRect rcPane;
...
void CMySplitterWnd::SetPane(int nIndex, CWnd* pPaneWnd)
{
	CRect rcPane;
...
CWnd* CMySplitterWnd::GetPane(int nIndex) const
{
	ASSERT((nIndex >= 0) && (nIndex < m_nPanes));
	return m_pane[nIndex];
}

void CMySplitterWnd::SetActivePane(int nIndex)
{
	ASSERT((nIndex >= 0) && (nIndex < m_nPanes));
	m_pane[nIndex]->SetFocus();
}

CWnd* CMySplitterWnd::GetActivePane(int* pIndex) const
{
...
void CMySplitterWnd::SetPaneSizes(const int* sizes)
{
...
void CMySplitterWnd::GetPaneRect(int nIndex, CRect& rcPane) const
{
	ASSERT(nIndex >= 0 && nIndex < m_nPanes);
...
void CMySplitterWnd::GetBarRect(int nIndex, CRect& rcBar) const
{
	ASSERT(nIndex > 0 && nIndex < m_nPanes);
	GetClientRect(&rcBar);
	if (m_nOrientation == SSP_HORZ)
...
void CMySplitterWnd::ResizePanes()
{
	int i;
	CRect rcOuter;
...
void CMySplitterWnd::InvertTracker()
{
	CDC* pDC = GetDC();
...
BOOL CMySplitterWnd::OnNcCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
void CMySplitterWnd::OnWindowPosChanging(WINDOWPOS FAR* lpwndpos) 
{

...
public:
	...
	BOOL SetItemCached(int nItem, BOOL bCached);
	BOOL GetItemCached(int nItem) const;
	WORD GetItemMode(int nItem) const;
	BOOL SetItemMode(int nItem, WORD wMode);
...
public:
	...
	void Sort(void);
	int SafeFind(LPCTSTR lpszItem, int nStart = 0);
	int SafeSearch(CString strItem, int nStart, BOOL bExactStart = FALSE, BOOL bWildCards = FALSE);
};

...
**
** Modifications Copyright (C) 2004-2006 Bender
**  
...
void CRichEditExCtrl::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if(m_pDocument != NULL)
	{
		...
		else if(CRichFragment* pFrag = PointToFrag(point))
		{
			...
			if(m_bDefaultLink && (pFrag->m_pElement->m_sLink.Find(_T("http://")) == 0) || (pFrag->m_pElement->m_sLink.Find(_T("ftp://")) == 0) || (pFrag->m_pElement->m_sLink.Find(_T("www.")) == 0))
			{
				
					LinkWarning warning;
					warning.m_strURL = pFrag->m_pElement->m_sLink;
...
void CRichEditExCtrl::Layout(CDC* pDC, CRect* pRect)
{
	...
	for(POSITION pos = m_pDocument->GetIterator() ; pos ;)
	{
		...
		{
			// -.,!:;)}]?
			if(*pszText != ' ' && *pszText != '-' && *pszText != '.' && *pszText != ',' 
				&& *pszText != '!' && *pszText != ':' && *pszText != ';' 
				&& *pszText != '\t' && *pszText != 0) continue;
...
void CRichEditExCtrl::CopySelection()
{
	...
	for(int nFragment = m_pSelAbsStart.nFragment ; nFragment <= m_pSelAbsEnd.nFragment ; nFragment++)
	{
		...
		if(pFragment->m_nLength == 0)
		{
			...
			if ( pFragment->m_pElement->m_nType == retBitmap)
			{
				
				str += pFragment->m_pElement->m_sText;
			}
